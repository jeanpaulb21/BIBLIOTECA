...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Función para mostrar formulario según sección
  function mostrarFormulario(seccion) {
    ...
    if (seccion === "libros") {
      formularioHTML = `
        <div class="container mt-4">
          ...
          <form id="form-agregar-libro" class="mx-auto" style="max-width: 500px;">
            <div class="mb-3">
    ...
    // Manejo básico de submit para los formularios
    const form = panelContainer.querySelector("form");
    if (form) {
      form.addEventListener("submit", (e) => {
        e.preventDefault();
        alert(`${seccion.charAt(0).toUpperCase() + seccion.slice(1)} enviado correctamente!`);
        form.reset();
      });
    }
  }

  // Aquí enlazas tus botones con la función mostrarFormulario
  // Quito btnAgregarUsuarios porque no quieres esa opción
  const btnAgregarLibros = document.getElementById("btn-agregar-libros");
  ...
  if (btnAgregarLibros) {
    btnAgregarLibros.addEventListener("click", () => mostrarFormulario("libros"));
  }
